...
class autRobot:
	def __init__(self):
		...
		self.outputRight = 0
		self.outputLeft = 0
		self.previousValueLeft = 0
		self.previousValueRight = 0
		self.Ts = 0.01
	...
	def joystickMode(self):
		...
		if(ch == 'o' or ch == 'O'):
			...
			if(self.lSpeed < MOTOR_SPEED_MAX or self.rSpeed < MOTOR_SPEED_MAX):
				self.lSpeed = min(MOTOR_SPEED_MAX, self.lSpeed + MOTOR_SPEED_STEP) + self.outputLeft
				self.rSpeed = min(MOTOR_SPEED_MAX, self.rSpeed + MOTOR_SPEED_STEP) + self.outputRight
				self.setMotorSpeeds(self.rSpeed, self.lSpeed)
		elif(ch == 's' or ch == 'S'):                    
			if(self.lSpeed > -1*MOTOR_SPEED_MAX or self.rSpeed > -1*MOTOR_SPEED_MAX):
				self.lSpeed = max(-1*MOTOR_SPEED_MAX, self.lSpeed - MOTOR_SPEED_STEP) + self.outputLeft
				self.rSpeed = max(-1*MOTOR_SPEED_MAX, self.rSpeed - MOTOR_SPEED_STEP) + self.outputRight
				self.setMotorSpeeds(self.rSpeed, self.lSpeed)                           
		elif(ch == 'a' or ch == 'A'):                    
	...
	def PID(self, Kp, Ki, Kd, tolerance):
	    while True:
	        # start = time.time()
	        # must be deleted to PID works
	        # simulated error since encoder was not available 
	        # robot.motorShaftLeft = robot.motorSpeedLeft_d - .4*robot.motorShaftLeft
	        ...
	        desiredValue = 200
	    	errorLeft = desiredValue - previousValueLeft
	    	errorRight = desiredValue - previousValueRight
	    	integralLeft += errorLeft
	        integralRight += errorRight
	        derivativeLeft = robot.motorShaftLeft - previousValueLeft
	        derivativeRight = robot.motorShaftRight - previousValueRight
	        if(errorleft > tolerance or errorleft < -tolerance):        
		        self.outputLeft = Kp*erroleft + Ki*integralLeft + Kd*derivativeLeft
		    else:
		    	integralLeft = 0
		    	self.outputLeft = 0
		    if(errorRight > tolerance or errorRight < -tolerance):    
		        self.outputRight = Kp*errorRight + Ki*integralRight + Kd*derivativeRight
		    else:
		    	integralRight = 0
		    	self.outputRight = 0    
	        previousValueLeft = robot.motorShaftLeft 
	        previousValueRight = robot.motorShaftRight
	        # end = time.time()
	        #time.sleep(self.Ts-end+start)
########### End of class ###########
